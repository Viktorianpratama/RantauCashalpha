...
export default (supabase) => {
  ...
  router.post('/add-payment', async (req, res) => {
    ...
    try {
      // Pastikan userId valid dan dapatkan data penghuni
      ...
      const { data: payment, error: paymentError } = await supabase
        ...
        .select('id, amount, status, due_date, user_id, room_id')
        .single();
      ...
      // Tambahkan nama penghuni ke respons
      res.status(201).json({
        message: 'Payment added successfully',
        payment: {
          ...payment,
          user_name: user.name, // Tambahkan nama penghuni
        },
      });
    } catch (error) {
      console.error('Error processing payment:', error);
